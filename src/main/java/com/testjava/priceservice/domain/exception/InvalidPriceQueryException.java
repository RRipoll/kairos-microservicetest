...
/**
 * Exception thrown when a price query violates domain business rules. This exception represents
 * invalid input at the domain level.
 */
public class InvalidPriceQueryException extends PriceServiceException {
  ...
  public InvalidPriceQueryException(
      final String field, final Object value, final String message, final Throwable cause) {
    super(
        String.format(ValidationMessages.INVALID_PRICE_QUERY_FORMAT, field, value, message), cause);
    this.field = field;
    this.value = value;
  }
